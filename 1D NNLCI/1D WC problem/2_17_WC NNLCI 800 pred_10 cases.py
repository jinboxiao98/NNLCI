...
# --- 配置 ---
ROOT_DIR = "./"
MODEL_FILENAME = 'best_model_800_40case.pth'
...
def recalculate_training_scalers(root_dir):
    ...
    # --- 🔥 修改点: 训练集索引必须与 train.py 一致 (只取10个) ---
    indices = np.arange(0, 400, 40) # 0, 40, 80 ... 360 (共10个)
    print(f"ℹ️ Using {len(indices)} training cases for scaler calculation.")
...
def predict_case(model, case_index, root_dir, shift_val, lb, ub):
    ...
    # 加载单个测试样本
    # 注意：这里加载的是原始全集，可以通过 index 任意选择测试对象
...
if __name__ == "__main__":
    ...
    # 这里会调用修改后的函数，重新计算适合 10 个样本的 scalers
    ...
    # 3. 选择 Case (建议选一个没有参与训练的 Case 来测试泛化能力，例如 55)
    TEST_CASE_IDX = 55 
