...
DATA_DIR = "./data" 
MODEL_FILENAME = 'best_model_lax_final.pth' # 新模型名区分
...
print(f"🚀 Training Lax (Window={WINDOW_SIZE}, Z-Score, GELU) on: {DEVICE}")
...
def load_lax_data(data_dir, mode='train'):
    ...
    mean = np.mean(in1, axis=(0, 1))
    std  = np.std(in1, axis=(0, 1))
    print(f"📊 Stats:\n   Mean: {mean}\n   Std : {std}")
    ...
    # Target 切片 (Trim = 5//2 = 2)
    trim = WINDOW_SIZE // 2
    target_slice = target_norm[:, trim:-trim, :]
    
    # Flatten
    # Input: Window(5) * Var(3) * Fidelity(2) = 30 features
