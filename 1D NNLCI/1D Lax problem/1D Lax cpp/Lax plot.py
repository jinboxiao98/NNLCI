...
def get_plot_data(case_id):
    ...
    # 1. 查找文件
    patterns = [
        f"./data/pred_{case_id}_{GRID_HF}.dat",      # pred_P3.8_800.dat
        f"./data/pred_Lax_{case_id}_{GRID_HF}.dat",  # pred_Lax_P3.8_800.dat
        f"./data/pred_Lax{case_id}_{GRID_HF}.dat",   # pred_LaxP3.8_800.dat
...
# ==========================================
# 3. 绘图主程序 (Main No Zoom)
# ==========================================
def plot_lax_main(case_id):
    print(f"🎨 Plotting Lax Main (No Zoom) for {case_id}...")
    ...
    for i in range(3):
        ...
        # 1. Low-Fidelity (底层)
        l1, = ax.plot(x, lf[:, i], color=C_LF, linestyle='-.', linewidth=1.0, label='Low-Fidelity Input', zorder=1)
        # 2. High-Fidelity (中层)
        l2, = ax.plot(x, gt[:, i], color=C_HF, linestyle='-', linewidth=1.0, label='High-Fidelity Solution', zorder=2)
        # 3. NNLCI (顶层)
        l3, = ax.plot(x, pred[:, i], color=C_NN, linestyle='--', linewidth=1.0, label='NNLCI Prediction', zorder=3)
        
        ax.set_xticks([0.0, 0.5, 1.0])
        ax.set_xlim(0, 1.0)
        ax.locator_params(axis='y', nbins=5)
        # 统一标题风格：8.5pt, Times New Roman
        ax.set_title(titles[i], pad=6, fontsize=8.5)
    ...
    save_path = f"Lax_Main_{case_id}.pdf"
