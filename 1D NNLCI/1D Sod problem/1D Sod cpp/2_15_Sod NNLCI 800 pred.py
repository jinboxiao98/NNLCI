...
# --- 配置 ---
DATA_DIR = "./data"
MODEL_FILENAME = 'best_model_sod_final.pth'
...
# --- 1. 获取训练 Scaling 参数 (Z-Score) ---
def get_scalers(data_dir):
    print("🔄 Calculating Z-Score params from training data...")
    files = glob.glob(os.path.join(data_dir, f"train_P*_{GRID_HF}.dat"))
    
    if not files: raise FileNotFoundError("No training data!")
...
def evaluate(model, mean, std):
    ...
    for hf_path in tqdm(files, desc="Predicting"):
        ...
        for v in range(3):
            plt.subplot(1,3,v+1)
            plt.plot(x_target, truth[:,v], 'k-', alpha=0.3, label='GT')
            plt.plot(x_target, interp_l1[:,v], 'g--', alpha=0.5, label='Low-Fi')
            plt.plot(x_target, pred_full[:,v], 'r-', label='NNLCI')
            plt.title(f"{['Den','Vel','Pres'][v]} L2:{l2[v]:.2e}")
            if v==0: plt.legend()
        plt.tight_layout()
        plt.savefig(f"results_final/{p_str}.png")
        plt.close()
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print(df.to_string(index=False, float_format=lambda x: "{:.2e}".format(x)))
    print("-" * 80)
