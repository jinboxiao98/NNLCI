...
struct SimParams {
    ...
    double rbdry = 1.0;
    double gam = 1.4;
    double CFL = 0.3; // 对于 1 阶格式，0.3 是非常安全的
    double t_final = 0.2;
    int order = 3;       // 1 = Low Order, 3 = High Order
    double p_left = 1.0; // Perturbed parameter
};

SimParams params;
double char_speed;

// Dynamic State Arrays: [index][variable][moment]
// variable: 0=density, 1=momentum, 2=energy
// moment: 0=0th(value), 1=1st(slope), 2=2nd(curve), 3=mean
...
int main(int argc, char* argv[]) {
    // 1. Parse Arguments: ./solver [nx] [p_left] [order] [output_id]
    ...
    if (argc >= 5) {
        ...
        std::cout << "Usage: ./solver [nx] [p_left] [order] [output_id]" << std::endl;
        params.nx = 100;
    }
    ...
    if (params.order == 3) {
        ...
        // For 1st order, ensure initial reconstruction (0th moment = mean)
    ...
    while (T < params.t_final) {
        ...
        if (params.order == 3) {
            ...
            // --- Low Order: Forward Euler (1st Order) ---
            ...
            // 2. Force reconstruction for 1st order
            // CRITICAL FIX: Set 0th moment (value) to Mean, and slopes to 0
            for (int i = 0; i < params.nxghst; i++)
                for (int k = 0; k < 3; k++) {
                    u_m[i][k][0] = u_m[i][k][3]; // <--- Value = Mean (Fixes NaN)
                    u_m[i][k][1] = 0.0;          // Slope = 0
                    u_m[i][k][2] = 0.0;          // Curvature = 0
                }
    ...
    // 4. Write Output (Using Mean Values)
    // Add "sol_" prefix as originally intended, let python handle renaming
...
// --- Implementation Details (Copied from previous valid logic) ---
...
void init(double dx) {
    for (int i = 0; i < params.nx + 1; i++) {
        ...
        if (x <= 0.5) {
            u[mod(i)][0][3] = 1.0;
            u[mod(i)][1][3] = 0.0;
            u[mod(i)][2][3] = params.p_left / (params.gam - 1.0);
        } else {
            u[mod(i)][0][3] = 0.125;
            u[mod(i)][1][3] = 0.0;
            u[mod(i)][2][3] = 0.1 / (params.gam - 1.0);
        }
    }
    for (int i = 0; i < params.nxghst; i++)
        for (int k = 0; k < 3; k++)
...
void adv_fw(double dx, double dt, double tau) {
    ...
    for (int k = 0; k < 3; k++) {
        for (int i = 0; i < params.nx; i++) {
            // Use u_m[...][0] (Value), [1] (Slope), [2] (Curve)
            // For Order 1: Slope=0, Curve=0, Value=Mean (Fixed now)
            double flux_diff = 
                LF_flux(
...
void limiting(double dx, double dt) {
    ...
    for (int i = 0; i < params.nx; i++) {
        for (int k = 0; k < 3; k++) {
            ...
            if (fabs(cell_l[0] - c_0) / dist[0] <= fabs(cell_l[1] - c_0) / dist[1]) { c_l = cell_l[0]; d_l = dist[0]; }
            else { c_l = cell_l[1]; d_l = dist[1]; }

            if (fabs(cell_r[0] - c_0) / dist[0] <= fabs(cell_r[1] - c_0) / dist[1]) { c_r = cell_r[0]; d_r = dist[0]; }
            else { c_r = cell_r[1]; d_r = dist[1]; }
